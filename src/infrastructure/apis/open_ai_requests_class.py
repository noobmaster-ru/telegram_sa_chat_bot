    ...
    async def classify_photo_order(
        ...
        return await self._classify_photo(
            prefix_message=(
                ...
                "Также считай со скрина ЦЕНУ НАШЕГО ТОВАРА в ₽ (цена должна находиться рядом с фотографией нашего товара)\n\n"
                "Ответ выдай в JSON: \{'is_order': bool, 'price': int|None\}"
                "Если на скриншоте нет цены, то в 'price' запиши None"
            ),
