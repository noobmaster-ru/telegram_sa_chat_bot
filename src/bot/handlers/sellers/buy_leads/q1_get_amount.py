...
from src.bot.keyboards.reply.menu import kb_menu, kb_buy_leads
...
async def unknown_text(message: Message, state: FSMContext):
    await message.answer(
        f"Пожалуйста, нажмите на кнопку {constants.SELLER_MENU_TEXT[1]}",
...
async def process_leads_amount(
    ...
    async with db_session_factory() as session:
        ...
        if user is None:
            await message.answer(
                "Не удалось найти ваш профиль пользователя. "
