...
class GoogleSheetClass():
    ...
    def add_new_buyer(
        ...
        Формат таблицы:
        Ссылка на ник | Дата первого | Дата последнего | Артикул | Согласен на условия | Подписка на канал | Заказ сделан | Отзыв оставлен | ШК разрезаны |  Реквизиты | Номер телефона | Сумма | Выплата произведена
        """
        ...
        new_row = [user_link, telegram_id, now, now, nm_id, status_agree, status_subscribe_to_channel, status_order, status_order_received, status_feedback, status_shk, requisites, phone_number, bank, amount, paid]
    ...
    def update_buyer_button_status(
        ...
        new_col_map = {k: keys[i+5] for i, k in enumerate(col_map) if i < len(keys)-5} # тк с 5 индекса начинаются условия наши
        
        # находим строку пользователя
        for i, record in enumerate(records, start=2):
            if record.get(keys[1]) == telegram_id:
    
                col_name = new_col_map[button_name]
                col_index = sheet.find(col_name).col
                sheet.update_cell(i, col_index, value)
                
                # также обновим дату последнего сообщения
                self.update_buyer_last_time_message(telegram_id=telegram_id)
                break