    ...
    async def get_gpt_5_response_before_agreement_point(self, new_prompt: str) -> str:
        response = await self.client.chat.completions.create(
            messages=[
                {
                    "role": "system", 
                    "content": f"Ты мой помощник. Ты отвечаешь на сообщения людей в Telegram. Отвечай кратко, без своих размышлений на сообщения пользователей , исходя из инструкции: {self.instruction_str}. Попроси пользователя согласиться на наши правила из инструкции, нажав на кнопку Telegram ниже."
                },
                {
                    "role": "user", 
                    "content": new_prompt
    ...
    async def get_gpt_5_response_after_agreement_and_before_subscription_point(
        ...
        response = await self.client.chat.completions.create(
            messages=[
                {
                    "role": "system", 
                    "content": f"Ты мой помощник. Ты отвечаешь на сообщения людей в Telegram. Отвечай кратко, без своих размышлений на сообщения пользователей , исходя из инструкции: {self.instruction_str}. Попроси его подписаться на наш канал: {CHANNEL_NAME}"
                },
                {
                    "role": "user", 
                    "content": new_prompt
    ...
    async def get_gpt_5_response_after_subscription_and_before_order_point(self, new_prompt: str) -> str:
        response = await self.client.chat.completions.create(
            messages=[
                {
                    "role": "system", 
                    "content": f"Ты мой помощник. Ты отвечаешь на сообщения людей в Telegram. Отвечай кратко, без своих размышлений на сообщения пользователей , исходя из инструкции: {self.instruction_str}. Нужно проверить пользователя на то, заказал ли он наш товар из инструкции, если он написал, что 'заказал/а', то попроси его нажать на кнопку Telegram ниже"
                },
                {
                    "role": "user", 
                    "content": new_prompt
    ...
    async def get_gpt_5_response_after_order_and_before_receive_product_point(self, new_prompt: str) -> str:
        response = await self.client.chat.completions.create(
            messages=[
                {
                    "role": "system", 
                    "content": f"Ты мой помощник. Ты отвечаешь на сообщения людей в Telegram. Отвечай кратко, без своих размышлений на сообщения пользователей , исходя из инструкции: {self.instruction_str}. Нужно проверить пользователя на то, получил ли он наш товар из инструкции, если он написал, что 'получил/а', то попроси его нажать на кнопку Telegram ниже"
                },
                {
                    "role": "user", 
                    "content": new_prompt
    ...
    async def get_gpt_5_response_after_receive_product_and_before_feedback_check_point(self, new_prompt: str) -> str:
        response = await self.client.chat.completions.create(
            messages=[
                {
                    "role": "system", 
                    "content": f"Ты мой помощник. Ты отвечаешь на сообщения людей в Telegram. Отвечай кратко, без своих размышлений на сообщения пользователей , исходя из инструкции: {self.instruction_str}. Нужно проверить пользователя на то, поставил ли он отзыв, если он написал, что 'оставил/а', то попроси его нажать на кнопку Telegram ниже"
                },
                {
                    "role": "user", 
                    "content": new_prompt
    ...
    async def get_gpt_5_response_after_feedback_and_before_shk_check_point(self, new_prompt: str) -> str:
        response = await self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system", 
                    "content": f"Ты мой помощник. Ты отвечаешь на сообщения людей в Telegram. Отвечай кратко, без своих размышлений на сообщения пользователей , исходя из инструкции: {self.instruction_str}. Нужно проверить пользователя на то, разрезал ли он этикетки(ШК) товара, если он написал, что 'разрезал/а', то попроси его нажать на кнопку Telegram ниже"
                },
                {
                    "role": "user", 
                    "content": new_prompt
