...
from src.core.di import setup_container
...
async def main():
    ...
    # добавляем redis в GoogleSheetClass через DI
    gs: GoogleSheetClass = await container.get(GoogleSheetClass)
    gs.redis_client = redis 
    ...
    # загружаем упорядоченный список артикулов для кэшбека из google_sheets в redis
    await gs.load_nm_ids_ordered_list_into_redis(
        sheet_name=constants.ARTICLES_SHEET_STR,
        REDIS_KEY_NM_IDS_ORDERED_LIST=constants.REDIS_KEY_NM_IDS_ORDERED_LIST,
    )
    # ---------- OpenAI ----------
    instruction_template = await gs.get_instruction_template(constants.INSTRUCTION_SHEET_NAME_STR)
    client_gpt_5: OpenAiRequestClass = await container.get(OpenAiRequestClass)
    client_gpt_5.instruction_template = instruction_template
